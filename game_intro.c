...
/*8x8 fonts for all the ASCII characters.*/
...
static char string0_to_print[] = "TETRIS";
/*Display Buffer to hold the text data*/
static uint8_t display_buffer[BUFF_ROW][BUFF_COL];



static void staticText(char *str, uint16_t str_len, uint8_t color);
static void printBuff(uint8_t color);
static void copyStatTextToBuff(char *str, uint16_t str_len);
...
void introAnimation()
{
     for(uint8_t loop = 0; loop < 3; loop++)
     {
     	...
     	{	
     		staticText(string0_to_print, sizeof(string0_to_print), BLACK + color);
     		usleep(100000);
     	}
     }
     
     do{
     	staticText(string0_to_print, sizeof(string0_to_print), BLACK);
     	usleep(100000);
     	staticText(string0_to_print, sizeof(string0_to_print), WHITE);
     	usleep(100000);
     }while(getchar());
